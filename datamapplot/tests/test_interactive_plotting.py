...
def validate_html_structure(html_content: str) -> dict:
    ...
    results = {
        ...
        "has_deck_container": 'id="deck-container"' in html_content
        or "id='deck-container'" in html_content,
        ...
        "has_style_tags": "<style" in html_content.lower()
        or "stylesheet" in html_content.lower(),
    }
...
def test_interactive_cord19(
    examples_dir,
    mock_image_requests,
    change_np_load_path,
    mock_interactive_save,
    mock_bz2_open,
    mock_display,
    mock_gzip_open,
    html_dir,
    ...
    assert offline_validation[
        "is_valid"
    ], f"Offline data validation failed: {offline_validation}"


@pytest.mark.interactive
def test_interactive_arxiv_ml(
    examples_dir,
    mock_image_requests,
    change_np_load_path,
    mock_interactive_save,
...
@pytest.mark.interactive
def test_interactive_arxiv_ml_topic_tree(
    examples_dir,
    mock_image_requests,
    change_np_load_path,
    mock_interactive_save,
...
class TestRenderHtmlBasic:
    ...
    def test_render_html_inline_basic(self, simple_point_data, simple_label_data):
        ...
        assert inline_validation[
            "is_valid"
        ], f"Inline data validation failed: {inline_validation}"
...
class TestRenderHtmlAdvanced:
    ...
    def test_render_html_with_histogram(
        self, point_data_with_extras, label_data_with_polygons
    ):
    ...
    def test_render_html_with_custom_css(
        self, point_data_with_extras, label_data_with_polygons
    ):
    ...
    def test_render_html_with_custom_js(
        self, point_data_with_extras, label_data_with_polygons
    ):
    ...
    def test_render_html_with_on_click(
        self, point_data_with_extras, label_data_with_polygons
    ):
...
def mock_interactive_save(monkeypatch):
    ...
    def _mock_save(html_output_dir):
        ...
        monkeypatch.setattr(
            "datamapplot.interactive_rendering.InteractiveFigure.save", save
        )
